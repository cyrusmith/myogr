# исправленный модуль StElsewhere, на данный момент не работает сохранение новых ассоциаций в таблицу.
module StElsewhere
  ...
  def has_many_elsewhere(association_id, options = {}, &extension)
    association_class = options[:class_name].to_s.classify.constantize || association_id.to_s.classify.constantize
  ...
  def collection_accessor_methods_elsewhere(association_id, association_class, through)
    ...
    association_class_singular = association_class.to_s.singularize
    association_class_plural = association_class.to_s
    through_association_singular = through.to_s.singularize
    my_class       = self
    my_foreign_key = self.to_s.foreign_key
    target_association_class = association_class_singular.classify.constantize
    target_association_foreign_key = association_class.to_s.singularize.foreign_key
    ...
    define_method("#{association_plural}") do
      ...
      through_associations.collect{|through_association| through_association.send("#{association_class_singular.underscore}")} || []
    end
  ...
  def associations_to_association_ids(associations)
    ...
    if associations && !associations.empty?
      ...
      ids = case association_class
              when "String"
                if associations.any? { |a| a.to_i == 0 }
                  associations
                else
